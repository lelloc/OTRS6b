...
sub Run {
    ...
    my $ExcludeQueue = $Param{JobConfig}->{'ExcludeQueue'};
    $Self->{CommunicationLogObject}->ObjectLog(
            ObjectLogType => 'Message',
            Priority      => 'Debug',
            Key           => 'Kernel::System::PostMaster::Filter::ExternalTicketNumberRecognition',
            Value         => "Exclude Queue: $ExcludeQueue",
        );
        
    #split all indicated queues into an array
    my @ExcludeQueues;
    @ExcludeQueues = split ';', $ExcludeQueue; 
        
	#list all array's elements, looking for Queues' ID and exclude them (delete from getAllQueue result)
    foreach my $EscludiCoda(@ExcludeQueues){     
    	my $QueueID = $QueueObject->QueueLookup( Queue => $EscludiCoda );
    	delete $Queues{ $QueueID };
    }
    #extract the keys values from hash and put it into an array
    my @SearchQueue = keys %Queues;
    
    #remove the queue indicated into parameter, otherwise add all queues except the queue indicated into parameter
    push @{ $Query{QueueIDs} }, @SearchQueue;
